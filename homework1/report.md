...
1. 使用 int i 表示從 0 到 2^n - 1 的所有組合（用位元表示子集合選擇狀態）。
2. i & (1 << j) 檢查第 j 個元素是否要放進當前的子集合中：
如果是 1，表示選取；如果是 0，表示不選。  
3. 輸出總共 2^n = 8 個子集合。   
...
程式透過整數的位元編碼方式，有效率且簡潔地列舉了所有子集合，具備高可讀性與高擴展性，是處理子集合相關問題的一種實用且經典的做法。

## 申論及開發報告

### 選擇位元運算法的原因

在本程式中，使用位元運算法實現輸出集合的所有子集合主要原因如下：

1. **簡潔且效率高**  
   使用 int 整數來代表每一種子集合，整數從 0 到 2^n - 1；每個位元對應一個元素是否出現在集合中。  

2. **避免遞迴，邏輯清楚、時間複雜度佳**  
   位元方法可以完全用雙重迴圈實作，簡單好推演每個子集合最多含有 n 個元素，時間複雜度比遞迴低。
